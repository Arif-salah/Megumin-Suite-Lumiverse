...
// ── The envelope ─────────────────────────────────────────────────────────────
//
// { __rid, __type, __data } — the caller's payload NESTED, never spread.
//
// It was flat once: { __rid, type, ...data }. That works right up until a
// payload carries a field the envelope also uses, and then the payload wins and
// the message is routed somewhere else entirely. It happened with ComfyUI's
// /view?type=temp: the image request went out with type "temp" instead of
// "comfy:image", the router could not place it, and the finished picture was
// never fetched. The error even said so — Unknown request type "temp" — but it
// reads as a ComfyUI problem rather than a protocol one.
//
// Nesting means a payload can carry any field name at all, including __rid and
// __type, and routing is unaffected.
...
export function call(type, data = {}, { timeoutMs = DEFAULT_TIMEOUT_MS } = {}) {
    ...
    return new Promise((resolve, reject) => {
        ...
        sendToBackend({ __rid: rid, __type: type, __data: data });
    });
}
...
export function notify(type, data = {}) {
    ...
    sendToBackend({ __type: type, __data: data });
}
