...
// the other side.
//
// The payload arrives NESTED under __data rather than spread alongside the
// routing fields. Flat, a payload carrying its own `type` overwrote the route —
// see the note in frontend/bridge.js for the request that went missing.
//
...
export function push(type, data = {}, userId = undefined) {
    // Nested for the same reason requests are — see frontend/bridge.js. There is
    // only one push today and its payload is ours, but the collision that broke
    // image downloads was in code just as safe-looking.
